...
public class YogaLayout extends ViewGroup {
  private final Set<ViewNodePair> mYogaNodes;
  protected final YogaNode mYogaNode;
  ...
  protected float getHorizontalScalingFactor() {
    return 1f;
  }

  protected float getVerticalScalingFactor() {
    return 1f;
  }
  ...
  private boolean containsNode(YogaNode node) {
    for (ViewNodePair pair : mYogaNodes) {
        if (pair.node == node) {
          return true;
        }
    }
  ...
  private boolean containsView(View child) {
    for (ViewNodePair pair : mYogaNodes) {
      if (pair.view == child) {
        return true;
      }
  ...
  /**
   * Like addView but doesn't actually add the view to the hierarchy. The node
   * is added to the managed set and made a child of the layout node. This is useful
   * if there are intermediary views in the hierarchy that aren't represented by yoga nodes.
   * <p>
   * Effectively this method behaves like how the VirtualYogaLayout does with its addView method.
   *
   * @param node  The Yoga node belonging to the view
   * @param child The view to add
   */
  public void addNodeWithView(YogaNode node, View child) {
    if (!containsNode(node)) {
      mYogaNodes.add(new ViewNodePair(child, node));
    }
  ...
  public void addView(View child, int index, ViewGroup.LayoutParams params) {
    ...
    if (child instanceof VirtualYogaLayout) {
      ...
      if (!containsView(child)) {
        childNode = ((VirtualYogaLayout) child).getYogaNode();
        mYogaNodes.add(new ViewNodePair(child, childNode));
  ...
  void addViewNodePair(ViewNodePair pair) {
    if (!containsView(pair.view) & mYogaNodes.add(pair)) {
      addView(pair.view);
    }
  ...
  public void removeAllViews() {
    for (Iterator<ViewNodePair> it = mYogaNodes.iterator(); it.hasNext(); ) {
      removeFromYogaTree(it, false);
    }
  ...
  public void removeFromParentView() {
    ...
    //If this layout is not parented by another layout, then remove
    //the node from the tree manually.
  ...
  private void removeFromYogaTree(Iterator<ViewNodePair> it, boolean inLayout) {
    ViewNodePair pair = it.next();
  ...
  private void applyLayoutRecursive(YogaNode node, float xOffset, float yOffset) {
    if (node.getDisplay() != YogaDisplay.NONE) {
      Object data = node.getData();
      float left = xOffset + (node.getLayoutX() * getHorizontalScalingFactor());
      float top = yOffset + (node.getLayoutY() * getVerticalScalingFactor());
      if (data instanceof View && data != this) {
        View view = (View) data;
        if (view.getVisibility() == GONE) {
          return;
        } else if (view.getLayoutParams() instanceof YogaLayout.LayoutParams) {
          view.measure(
            View.MeasureSpec.makeMeasureSpec(
							Math.round(node.getLayoutWidth() * getHorizontalScalingFactor()),
              View.MeasureSpec.EXACTLY),
            View.MeasureSpec.makeMeasureSpec(
					 		Math.round(node.getLayoutHeight() * getVerticalScalingFactor()),
              View.MeasureSpec.EXACTLY));

          int _left = Math.round(left);
          int _top = Math.round(top);
          view.layout(_left, _top, _left + view.getMeasuredWidth(), _top + view.getMeasuredHeight());
        }
      }
      ...
      for (int i = 0; i < childrenCount; i++) {
        ...
        } else if (data == this || !(data instanceof YogaLayout)) {
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    if (!(getParent() instanceof YogaLayout) && mYogaNode.getOwner() == null) {
      createLayout(
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    setMeasuredDimension(
      Math.round(mYogaNode.getLayoutWidth() * getHorizontalScalingFactor()),
      Math.round(mYogaNode.getLayoutHeight() * getVerticalScalingFactor()));
  }

  private void createLayout(int widthMeasureSpec, int heightMeasureSpec) {
    final float widthSize = MeasureSpec.getSize(widthMeasureSpec) * (1 / getHorizontalScalingFactor());
    final float heightSize = MeasureSpec.getSize(heightMeasureSpec) * (1 / getVerticalScalingFactor());
  ...
  protected static void applyLayoutParams(LayoutParams layoutParameters, YogaNode node, View view) {
    ...
    for (int i = 0; i < layoutParameters.numericAttributes.size(); i++) {
      ...
      } else if (attribute == R.styleable.yoga_yg_gapColumn) {
        node.setGap(YogaGutter.COLUMN, value);
      } else if (attribute == R.styleable.yoga_yg_gapRow) {
        node.setGap(YogaGutter.ROW, value);
      } else if (attribute == R.styleable.yoga_yg_gapAll) {
        node.setGap(YogaGutter.ALL, value);
    ...
    for (int i = 0; i < layoutParameters.stringAttributes.size(); i++) {
      ...
      if (value.equals("auto")) {
        if (attribute == R.styleable.yoga_yg_flexBasis) {
          ...
          node.setPositionAuto(YogaEdge.LEFT);
        } else if (attribute == R.styleable.yoga_yg_positionTop) {
          node.setPositionAuto(YogaEdge.TOP);
        } else if (attribute == R.styleable.yoga_yg_positionRight) {
          node.setPositionAuto(YogaEdge.RIGHT);
        } else if (attribute == R.styleable.yoga_yg_positionBottom) {
          node.setPositionAuto(YogaEdge.BOTTOM);
        } else if (attribute == R.styleable.yoga_yg_positionStart) {
          node.setPositionAuto(YogaEdge.START);
        } else if (attribute == R.styleable.yoga_yg_positionEnd) {
          node.setPositionAuto(YogaEdge.END);
        } else if (attribute == R.styleable.yoga_yg_positionHorizontal) {
          node.setPositionAuto(YogaEdge.HORIZONTAL);
        } else if (attribute == R.styleable.yoga_yg_positionVertical) {
          node.setPositionAuto(YogaEdge.VERTICAL);
        } else if (attribute == R.styleable.yoga_yg_positionAll) {
          node.setPositionAuto(YogaEdge.ALL);
        } else if (attribute == R.styleable.yoga_yg_width) {
          node.setWidthAuto();
        }
      }
      ...
      if (value.endsWith("%")) {
        ...
        if (attribute == R.styleable.yoga_yg_flexBasis) {
          ...
          node.setGapPercent(YogaGutter.COLUMN, numericValue);
        } else if (attribute == R.styleable.yoga_yg_gapRow) {
          node.setGapPercent(YogaGutter.ROW, numericValue);
        } else if (attribute == R.styleable.yoga_yg_gapAll) {
          node.setGapPercent(YogaGutter.ALL, numericValue);
        } else if (attribute == R.styleable.yoga_yg_height) {
  ...
  static class ViewNodePair {
  	View view;
  	YogaNode node;

    ViewNodePair(View view, YogaNode node) {
      this.view = view;
      this.node = node;
    }
  }
  ...
  public static class ViewMeasureFunction implements YogaMeasureFunction {
    ...
    public long measure(
      ...
      if (!(data instanceof View) || data instanceof YogaLayout) {
